...
function IncidentDetailPage() {
  ...
  if (incidentQuery.isPending) {
    return (
      <div className="content-area-scroll flex-1 min-h-0 overflow-y-auto p-6">
        <p className="text-ink/70">Loading incident…</p>
      </div>
    )
  }
  if (incidentQuery.isError || !incidentQuery.data) {
    return (
      <div className="content-area-scroll flex-1 min-h-0 overflow-y-auto p-6">
        <p className="text-ink/70">Couldn&apos;t load this incident.</p>
        <Link to="/incidents" className="text-sm text-natural hover:underline">
          ← Back to incidents
        </Link>
      </div>
    )
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-4 print:hidden">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 text-xl font-semibold text-ink">{inc.title}</h1>
          </div>
          <Button
            type="button"
            variant="secondary"
        ...
        <article className="space-y-6 border border-ink/20 bg-white p-6 print:border-0 print:p-0">
          <section>
            <p className="text-xs uppercase tracking-wide text-ink/60">Critical incident</p>
            <h2 className="mt-1 text-2xl font-semibold text-ink">{inc.title}</h2>
            ...
            <dl className="mt-4 grid gap-4 sm:grid-cols-3">
              <div>
                <dt className="text-xs text-ink/60">Affected population</dt>
                <dd className="text-sm text-ink">{inc.affected_population}</dd>
              </div>
              <div>
                <dt className="text-xs text-ink/60">Linked sessions</dt>
                <dd className="text-sm text-ink">{inc.linked_session_ids?.length ?? 0}</dd>
              </div>
              <div>
                <dt className="text-xs text-ink/60">Client</dt>
                <dd className="text-sm text-ink font-mono break-all">{inc.client_id}</dd>
              </div>
            </dl>
          </section>
          ...
          <section>
            ...
            <p className="mt-2 whitespace-pre-wrap text-sm text-ink/90">{inc.description}</p>
          </section>

          <section>
            <h3 className="text-sm font-semibold text-ink">Timeline</h3>
            {timelineQuery.isPending ? (
              <p className="mt-2 text-sm text-ink/60">Loading timeline…</p>
            ) : (
              <ol className="mt-3 space-y-3 border-l border-ink/20 pl-4">
                {timeline.map((event) => (
          ...
          {inc.resolution_notes && (
            <section>
              <h3 className="text-sm font-semibold text-ink">Resolution notes</h3>
              <p className="mt-2 whitespace-pre-wrap text-sm text-ink/90">{inc.resolution_notes}</p>
            </section>
          )}
        </article>

        <section className="border border-ink/20 bg-white p-4 print:hidden">
