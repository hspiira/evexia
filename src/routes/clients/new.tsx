    ...
    <div className="p-6 max-w-md">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <FormField label="Code (3–5 chars)" required error={fieldErrors.code} htmlFor="code">
          <Input id="code" value={code} onChange={(e) => setCode(e.target.value)} required className="rounded-none" />
        </FormField>
        ...
        <FormField label="Phone" error={fieldErrors["contact_info.phone"]} htmlFor="phone">
          <Input id="phone" value={phone} onChange={(e) => setPhone(e.target.value)} className="rounded-none" />
        </FormField>
        <div className="flex gap-2">
          <Button type="submit" disabled={loading} className="rounded-none">
            {loading ? "Creating…" : "Create client"}
          </Button>
          <Button type="button" variant="secondary" className="rounded-none" onClick={() => navigate({ to: "/clients" })}>
            Cancel
          </Button>
        </div>
      </form>
