...
function UserDetailPage() {
  ...
  if (!user) {
    return (
      <div className="p-8">
        <p className="text-fg">User not found.</p>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="border border-fg/30 rounded-none p-6 bg-surface/30">
        <h1 className="text-xl font-semibold text-fg">{user.email}</h1>
        <dl className="mt-4 grid gap-2 sm:grid-cols-2">
          <div>
            <dt className="text-sm text-fg/80">Status</dt>
            <dd><StatusBadge status={user.status} /></dd>
          </div>
          <div>
            <dt className="text-sm text-fg/80">Email verified</dt>
            <dd>{user.is_email_verified ? "Yes" : "No"}</dd>
          </div>
          <div>
            <dt className="text-sm text-fg/80">2FA</dt>
            <dd>{user.is_two_factor_enabled ? "On" : "Off"}</dd>
          </div>
          {user.last_login_at && (
            <div>
              <dt className="text-sm text-fg/80">Last login</dt>
