...
const columns = [
  {
    ...
    header: "Name",
    cell: (row: Service) => (
      <Link to="/services/$serviceId" params={{ serviceId: row.id }} className="text-[#8BA88B] hover:underline">{row.name}</Link>
    ),
  },
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link to="/services/new" className="inline-flex items-center justify-center h-9 px-4 bg-[#8BA88B] text-white font-medium rounded-none hover:bg-[#7a9a7a]">Add service</Link>
      </div>
