...
function EngagementDetailPage() {
  ...
  if (!engagementQuery.data) {
    return <p className="p-6 text-sm text-fg/60">Engagement not found.</p>
  }
      ...
      <div className="mx-auto max-w-4xl space-y-5">
        <header>
          ...
          <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h1 className="text-xl font-semibold text-fg">{engagement.name}</h1>
              <p className="mt-1 text-sm text-fg/70">
                ...
                {engagement.due_date
                  ...
                  : ""}
                {" · "}
                <span className="font-medium text-fg">{engagement.status}</span>
              </p>
              {engagement.description ? (
                <p className="mt-2 text-sm text-fg/70">{engagement.description}</p>
              ) : null}
            </div>
        ...
        <section className="border border-fg/20 bg-white p-4">
          ...
          {timelineQuery.isPending ? (
            <p className="mt-2 text-sm text-fg/60">Loading…</p>
          ) : timeline.length === 0 ? (
            <p className="mt-2 text-sm text-fg/60">No events yet.</p>
          ) : (
            <ul className="mt-3 space-y-2 text-sm">
              {timeline.map((e) => (
                <li key={e.id} className="border-l-2 border-primary/40 pl-3">
                  ...
                  <p className="text-fg/80">{e.message}</p>
                </li>
              ))}
            </ul>
          )}
  ...
  return (
    <div
      className={`border p-3 ${
        highlight ? "border-amber-500/40 bg-amber-50" : "border-fg/20 bg-white"
      }`}
    >
      <div className="text-xs uppercase text-fg/60">{label}</div>
      <div className={`mt-1 text-lg font-semibold ${highlight ? "text-amber-700" : "text-fg"}`}>
        {value}
      </div>
    </div>
  )
  ...
  return (
    <section className="border border-fg/20 bg-white p-4 space-y-3">
      <header className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-fg">Deliverables</h2>
        <Button
          type="button"
          variant="secondary"
          size="sm"
      ...
      {open && (
        <div className="space-y-2 border border-dashed border-fg/30 p-3">
          <Input
            placeholder="Title"
            value={title}
      ...
      {loading ? (
        ...
        <p className="text-sm text-fg/60">No deliverables yet.</p>
      ) : (
        <ul className="divide-y divide-ink/10 border-t border-fg/10">
          {deliverables.map((d) => (
            <li key={d.id} className="flex items-center justify-between gap-3 py-2">
              ...
              <select
                ...
                className="h-8 border border-fg/30 bg-white px-2 text-xs rounded-none text-fg"
              >
...
function TimeEntriesSection({
  ...
  return (
    <section className="border border-fg/20 bg-white p-4 space-y-3">
      <header>
        <h2 className="text-sm font-semibold text-fg">Hours log</h2>
        <p className="text-xs text-fg/60">Hours roll up into the engagement totals.</p>
      </header>
      ...
      <div className="grid gap-2 sm:grid-cols-[8rem_5rem_1fr_10rem_auto] items-start">
        ...
        <select
          ...
          className="h-9 border border-fg/30 bg-white px-2 text-sm rounded-none text-fg"
        >
      ...
      {loading ? (
        ...
        <p className="text-sm text-fg/60">No time logged yet.</p>
      ) : (
        <table className="w-full border border-fg/10 text-sm">
          <thead className="bg-neutral-50 text-fg">
            <tr>
              <th className="border-b border-fg/10 px-2 py-1.5 text-left font-medium">Date</th>
              <th className="border-b border-fg/10 px-2 py-1.5 text-right font-medium">Hours</th>
              <th className="border-b border-fg/10 px-2 py-1.5 text-left font-medium">By</th>
              <th className="border-b border-fg/10 px-2 py-1.5 text-left font-medium">Note</th>
            </tr>
          </thead>
          <tbody>
            {entries.map((e) => (
              <tr key={e.id} className="text-fg/80">
                <td className="border-b border-fg/10 px-2 py-1.5">
                  {new Date(e.occurred_on).toLocaleDateString()}
                </td>
                <td className="border-b border-fg/10 px-2 py-1.5 text-right tabular-nums">
                  {e.hours.toFixed(2)}
                </td>
                <td className="border-b border-fg/10 px-2 py-1.5">{e.user_id}</td>
                <td className="border-b border-fg/10 px-2 py-1.5">{e.description ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
